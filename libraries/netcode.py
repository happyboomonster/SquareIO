...
def send_data(Cs,buffersize,data,ack="ACK"): #sends a packet of data as a string. Uses some basic error correction to lower the chances of disconnection
    datalen = justify(str(len(list(str(data)))),buffersize)
    data = str(data)
    while True:
        Cs.send(bytes(datalen,'utf-8')) #send the buffersize of our data
...
def recieve_data(Cs,buffersize,evaluate=False,returnping=False,ack="ACK"): #recieves a packet of data as a string. Uses some basic error correction to lower the chances of disconnection
    pingstart = time.time() #set a starting ping time
    while True:
        Nbuffersize = int(Cs.recv(buffersize).decode('utf-8')) #get our data's buffersize
        Cs.send(bytes(justify(ack,buffersize),'utf-8')) #send an acknowledge signal
